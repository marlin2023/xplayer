...
int XMessageQueue::push_front(ipc_msg_node_t *msg_node)
{
    ...
    this->msg_queue.push_front(msg_node);
    // alter node_count
...
ipc_msg_node_t* XMessageQueue::pop()
{
    pthread_mutex_lock(this->mutexLock);
    ...
    // pop a message node
    ipc_msg_node_t *poped_node = this->msg_queue.front();
